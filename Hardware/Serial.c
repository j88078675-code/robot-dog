...
//void Serial_SendString2(u8 *DAT, u8 len)
//{
//	u8 i;
//	for(i = 0; i < len; i++)
//	{
...
void Serial_SendNumber(uint32_t Number, uint8_t Length)
{
	...
	{
		Serial_SendByte1(Number / Serial_Pow(10, Length - i - 1) % 10 + '0');	//依次调用Serial_SendByte发送每位数字
	}
}
...
{
	return Serial_RxData1;			//返回接收的数据变量
}
uint8_t Serial_GetRxData3(void)
{
	return Serial_RxData3;			//返回接收的数据变量
}
...
void USART3_IRQHandler(void)
{
	...
	{
		Serial_RxData3 = USART_ReceiveData(USART3);				//读取数据寄存器，存放在接收的数据变量
		Serial_RxFlag3 = 1;										//置接收标志位变量为1
